...
export default function OurStory({ content }: OurStoryProps = {}) {
  const trustBadges = (content?.badges && content.badges.length > 0)
    ? content.badges
    : DEFAULT_BADGES;
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            <span className="text-gold text-xs font-semibold uppercase tracking-widest block">
              {content?.tagline || 'Our Roots & Heritage'}
            </span>
            ...
            <blockquote className="border-l-2 border-gold pl-6 py-4 bg-[#163322]/25 rounded-r-2xl pr-6 space-y-2">
              <p className="text-gold font-serif italic text-sm sm:text-base leading-relaxed">
                &ldquo;{content?.founder_quote || 'I strongly believe in one principle: If you can’t consume your own product every day, you probably shouldn’t be selling it. At Kindleaf, we drink the exact same tea we ship to you.'}&rdquo;
              </p>
              <cite className="block text-xs font-semibold text-slate-400 not-italic tracking-wider uppercase">
                — {content?.founder_name || 'Gaurav Singh, Founder'}
              </cite>
            </blockquote>
          </div>
